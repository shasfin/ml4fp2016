...
Now, we need to expand a program in the frontier again.  Let us assume that the least-cost program is \lstinline|map (const ?x) ?xs|.  There are two holes to fill in: \lstinline|?x| of type \lstinline|X| and \lstinline|?xs| of type \lstinline|List ?Y|.  Interestingly, we have only one option for the first hole, and so we take it: \lstinline|replicate|'s second argument \lstinline|x|:
\begin{lstlisting}[style=plain]
replicate n x = map (const x) ?xs
?p = map ?f ?xs :: List X
?f = const ?x :: ?Y -> X
...
\item[Successor] a program derived by filling in a hole of a partial program. In Section~\ref{Search space} we can find the rules according to which holes are expanded. 
\item[Best-first search] a frontier of programs with holes is maintained, one hole of the least-cost program is expanded in every iteration. The best-first search algorithm is defined in Section~\ref{Exploration} and several cost functions are presented in Section~\ref{Cost functions}.
