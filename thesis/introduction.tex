...
% The algorithm accepts accepts a list of input-output examples, the type of the program to be synthesised, and a library of components to use.  It, then, aims to synthesise a solution from the components in the library, such that it behaves according to the input-output examples.


%% \section{Summary}\label{Problem}


%% For that, we use a best-first-search algorithm (based on various cost functions).  Importantly, the best-first search

%% We use a simple enumerate-and-test approach. In order to avoid errors due to bad typing, we enumerate only well-typed programs. We rely on a large library of first- and higher-order components reflecting common computational patterns.
