...
\section{Problem}
Recall the example from Chapter~\ref{ch:introduction} where we wanted to synthesise \lstinline?replicate?. As our synthesis is type-driven, the type of \lstinline?replicate?, along with the I/O-examples, is part of the specification of the program. 
\begin{itemize}
\item recall example
\item Let us specify the target program by giving its type and two I/O-examples.
\begin{lstlisting}[style=plain]
replicate :: $\forall$X. Int -> X -> List X
replicate 3 1 = [1,1,1]
replicate 2 [] = [[],[]]
\end{lstlisting}
\item We also need a library of components, from which we are going to synthesise our program. Let us assume we have the standard list combinators \lstinline?map, foldr, foldl?, \lstinline?enumTo? and the higher-order function \lstinline?const? that always returns the first input.
\item Try to put components together in order to get a list.
\begin{lstlisting}[style=plain]
replicate n x = ...
\end{lstlisting}
We see that in order to get something of type \lstinline?List X?, we cannot use \lstinline?enumTo?, because it has type \lstinline?Int -> List Int?
\begin{lstlisting}[style=plain]
replicate n x =
\end{lstlisting}
\end{itemize}

Let us look at the example from Chapter~\ref{ch:introduction} more in detail. \lstinline?replicate?
