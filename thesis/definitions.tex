...
But also need another language: the language in which the library components are defined and the programs are evaluated on the input-output examples. We call this language \emph{internal language}. It extends System F with holes, parametric types and recursion.
...
Since the target language is a subset of the internal language, Section~\ref{Typing} and Section~\ref{Evaluation} show the typing and, respectively, the evaluation rules only for the internal language.
Section~\ref{Encodings} discusses how to encode integers and lists in the internal language. Section~\ref{Unification} addresses unification over the types of the internal language, needed to determine which library components can be used to fill in a hole of a particular type.
...
\paragraph{Target language} The target language of our synthesiser is a subset of the internal language. We already saw terms of this language in the \lstinline!replicate! example, but there all type applications were omitted for the sake of clarity.
...
Other algebraic datatypes, such as trees, can be easily encoded in the same style.
