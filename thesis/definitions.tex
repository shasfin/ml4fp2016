...
In this chapter we formally define our type-directed top-down synthesis procedure. We start with an intuitive description thereof based on an example and move on to the formal definitions, starting with the target programming language and the search space. Finally, we present some enhancements.
...
The syntax of the internal language and the target language is defined in Section~\ref{Term and types}.  For the sake of completeness, we also provide the syntax of System F, as the other two calculi build upon it.  Section~\ref{Evaluation} and Section~\ref{Typing} show the evaluation and, respectively, the typing rules only for the internal language, since the target language is a proper subset thereof.  The notation and the exposition closely follow the excellent book on type systems by Benjamin Pierce \cite{pierce2002types}.  We refer to the book for a thorough introduction to System F and to type systems in general.
