...
Alas, unification on the type system of System F is undecidable \cite{Huet1975}. Therefore we chose to restrict our type system to universally quantified types, that is types whose universal quantifiers are at the toplevel. For example, \lstinline?$\forall$X. $\forall$Y. X -> Y? is such a type, whereas \lstinline?Int -> ($\forall$X. X -> Int)? is not. This allows us to completely ignore universal types during unification. Instead, we represent universal types in the library as not universal types where we replace each universally quantified variable with a fresh hole. That is, the type of \lstinline?length? is stored in the library as \lstinline!List ?X -> Int!.
...
\section{Search}
\TODO{Find a general notation for the deuniversalise and dearrowise thing, it looks very confusing. Define it somewhere in the beginning and use always the same. A good way is to couple fresh(T) with c [T] [T] [T]. Apply fresh holes to the component, THEN unify.\\}
I can either say that I apply fresh holes to a library component until no universal quantifiers are left and then unify the type of this new component with the "goal type". I need it in three places:
\begin{enumerate}
\item in unification, when I say how I can unify universally quantified variables
\item in the problem definition, when I relate the goal type with the type of the programs.
\item in the successor rules, in the rule D-VarLib, when you say that you can replace a hole with a library component with the right type. Well, actually not the component, but the type application of the component to $\sigma$ of the fresh holes we applied to the component in order to make unification possible.
\end{enumerate}


Let $\mathcal{A}$ be the class of quantifier-free types. Try to do it with fresh holes and explicit type holes. You can also need a class of arrow- and quantifier-free types. Say then that the goal type has the form bla bla bla and introduce it like that.

Summary: in unification say that you have unification only for quantifier-free types. Say that this still allows to have universally quantified components and explain the trick with the fresh holes. In the problem definition: see above. In the successor rule: decompose type of component as $T_c = \forall X. \forall Y. T(X,Y)$, then take two fresh holes, unify $T(?X,?Y)$ with the type of the hole and instantiate the hole with $c\; [\sigma(?X)]\; [\sigma(?Y)]$.
\TODO{End of notation discussion\\}
After defining the target language, the type checking and the evaluation semantics, we are ready to formally define the problem.
...
D-VarLib and D-VarInp replace a hole $?x$ by a library component $c$ from the context $\Delta$ \TODO{type application. For universally typed $c$ it's not $c$ but $c\;[T_1]\;\ldots\;[T_k]$ for some types or fresh type holes}, or an input variable $i$ from the context $\Phi$, respectively, if they have the right type. The procedure fresh($T$) removes the universal quantifiers and replaces the bound type variables $X$ with fresh type holes $?X$ not used in $\Xi$.
The notation $\sigma(\Delta)$ denotes the application of the substitution $\sigma$ to all types contained in the context $\Delta$.\\
D-VarApp turns a hole into a term application of two fresh holes.\\
The rule D-App chooses a hole in the program and expands it according to one of the three rules above.
...
\AxiomC{$\sigma$ unifies $T$ with fresh($T_c$)}
	...
	\TODO{type application $c\;[T_1]\;\ldots\;[T_k]$ depending on $T_c$ and $\sigma$}}
\end{prooftree}
...
what we want to say in this section
\begin{itemize}
\item it's standard
\item it's best-first search
\item algorithm
\item there are two points that can be tweaked
\item stack vs queue expansion (mention it at least)
\item mention cost functions and refer to the next section
\end{itemize}


what do we want to have in this section
\begin{itemize}
\item section Best-first search $\longrightarrow$ standard enumeration. We can tweak it in two points: which hole first and what program first. Write about stack vs queue and refer to next section for cost functions. Or do another sections about it? Section or subsection? In best-first search or already in the search space?
\end{itemize}
