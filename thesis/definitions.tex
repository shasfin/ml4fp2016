...
We also need a library of components, from which we are going to compose our program. Let us assume we have the standard list combinators \lstinline?map, foldr, foldl? with \lstinline?enumTo?, the function that returns a list from $1$ up to its argument, and \lstinline?const?, that always returns its first argument along with the list constructors \lstinline?cons? and \lstinline?[]? and the integer constructors \lstinline?succ? and \lstinline?0?.

\begin{itemize}



\item Try to put components together in order to get a list.
Fix $X$ as a type variable and fix $n$ to be an Int and $x$ to be an $X$. Try to synthesise a \lstinline?List X?. Which components can you use in order to get a \lstinline?List X?? You cannot use \lstinline?enumTo?, because it only produces a list of integers, but all other possibilities are open. We could either fold  with an interesting function, or map some function, or even use \lstinline?const? with a smart first argument. But the first and easiest thing that has the type \lstinline?List X? is \lstinline?[]?.
\begin{lstlisting}[style=plain]
replicate n x = []
\end{lstlisting}
Cool, we can evaluate this program straight away... and see that is does not satisfy the I/O-examples. It means, we must try further.
...
?xs :: List ?Y
\end{lstlisting}
\lstinline!?Y! is a fresh variable.
We omit \lstinline?foldl? for brevity.
\begin{lstlisting}[style=plain]
...
Let us decide that the first one is the most promising. We now have two holes to fill in, a function that can take something and return an \lstinline?X? (note that it has to be possible for all possible instantiations of \lstinline?X? and note that we have only one value of type \lstinline?X?: \lstinline?x?, the second argument to \lstinline?replicate?.\\
What are the possibilities to instantiate \lstinline!?f!? Obviously, we cannot use \lstinline?map? or \lstinline?enumTo?, because they return lists. But all other possibilities are still open (again, we leave out \lstinline?foldl? for brevity).
\begin{lstlisting}[style=plain]
...
Again, from all programs generated so far (6 in total) we choose the most promising one. Let us decide that it is the last one. It has two holes to fill in. For the first hole, \lstinline!?x!, we have only one possibility. As this hole must be of type \lstinline?X?, the only thing we can take is the second argument of \lstinline?replicate?, \lstinline?x?.
...
Let us directly decide this is the most promising program so far (how to decide which program are we going to expand next is explained in Section~\ref{Cost functions}).
Like at the beginning, we have to generate a list. But this time we can choose what the elements of the list are, therefore we cannot rule out \lstinline?enumTo?. As you see, we have a lot of possibilities, starting with \lstinline?replicate n x = map (const x) []?, where we instantiate \lstinline!?xs! with \lstinline?[]?, and ending with
\begin{lstlisting}[style=plain]
...
?n :: Int
\end{lstlisting}
First we are going to evaluate the \emph{closed} program (that is, without holes) \lstinline?replicate n x = map (const x) []?... and see that it doesn't satisfy the I/O-examples.
...
This time, we got two closed programs that we are going to evaluate, \lstinline?replicate n x = map (const x) (enumTo 0)? and \lstinline?replicate n x = map (const x) (enumTo n)?. Evaluation shows that only the second one satisfies the I/O-examples.


\item Important difference between \lstinline?X? and \lstinline!?X!. The first one is fixed and cannot be instantiated with anything, the second one is a free variable that can be instantiated with anything we like.
\end{itemize}

This example show following concepts
\begin{itemize}
\item Hole: unknown part of a program, for which we only know the type and that we can expand
\item we maintain a frontier of programs with holes
\item from this frontier, we always chose the most promising one. What program is the most promising can be determined by a cost function (Section~\ref{Cost functions})
\item some programs are not worth expanding. For example, we see straight away that is does not make much sense to instantiate a hole with \lstinline!const ?x ?y!, because we could as well write \lstinline!?x! for it, which is always a shorter and preferable program. That is, some programs can be ruled out semantically (Section~\ref{Black list})
\item The search space is quite big even with so few components. Exponential.
\end{itemize}
...
\end{algorithm} 
%
\TODO{describe the other two no-same-component}
